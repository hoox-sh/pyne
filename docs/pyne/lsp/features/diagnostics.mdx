...
| `src/pynescript/langserver/features/diagnostics.py` | Lint conversion, C001/C003 noise filters, tags, `create_quick_fix` |
...
6. **Noise filters:** `C001` on names that already lack `_` (camelCase `fastMA`) is dropped; `C003` on block `if` (next non-empty line more indented) is dropped.
