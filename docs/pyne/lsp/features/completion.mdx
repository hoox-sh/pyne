...
Completion is catalog-driven, not evaluator-driven. The server loads builtin metadata (plaintext JSON in development, Fernet-encrypted in compiled binaries), then also offers **Pine keywords** (`PINE_KEYWORDS`) and **user `enum` types / members** collected from the workspace AST. Resolve re-hydrates documentation for a single item.
...
flowchart TD
  ...
  TR -->|module.| MOD[build_module_completion or enum members]
  TR -->|prefix text| ALL[keywords + user enums + fuzzy_filter]
...
3. Collect user enums from the cached AST (`collect_user_enums`).
4. If the last token contains `.` (e.g. `ta.` or `Side.`):
   - user-enum prefix → `build_enum_member_completion`
   - else `build_module_completion(module)` for catalog namespaces
5. Else merge `build_keyword_items`, `build_enum_name_items`, and `build_completion_list(prefix=prefix)`.
...
1. **No local function / variable completion yet** — user **enums** and **keywords** are included; local `myFunc` is not, unless it appears in metadata.
