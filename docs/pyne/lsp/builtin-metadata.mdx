...
| Function | Behavior |
| --- | --- |
| `get_metadata()` | Singleton cache; **plaintext JSON first**, else `.enc` decrypt, else `{}` |
...
`scripts/build/compile.py` → `encrypt_metadata()` / `_resolve_fernet_key()`:

1. Require `cryptography` and plaintext JSON.
2. Resolve key (first hit wins): env `CRYPTO_KEY` → `PYNESCRIPT_METADATA_KEY` → `METADATA_KEY` → existing `scripts/build/.metadata.key` → generate a new Fernet key and write that file (mode `0o600`).
3. Encrypt → `builtin_metadata.json.enc`.
4. Write truncated SHA-256 of plaintext to `.sha256`.

A random key is generated **only on first local encrypt** when no env/file key exists. For **byte-stable** encrypted artifacts across CI runs, supply a stable secret:
...
Both `get_metadata()` and `get_metadata_cached()` prefer **plaintext** when both artifacts exist (developer-friendly). Difference: `get_metadata()` returns `{}` on total failure; `get_metadata_cached()` raises `FileNotFoundError` if neither file is present.
