...
- Subclasses `pygls.lsp.server.LanguageServer` with `name="Pynescript"` and `version=__version__` from `src/pynescript/__about__.py` (**0.3.10**).
...
- Signature help and code action are **not** advertised (`config.py` omits them until handlers exist). `workspace/executeCommand` is registered but returns `None`.
