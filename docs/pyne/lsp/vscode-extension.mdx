...
| dependency | `vscode-languageclient` ^10 |
...
| **PYNE: Convert to Pine v6** | `pynescript.convertToV6` | Rewrite v1-v5 source toward v6 (LSP command or local Python) |
...
Site download: [https://hoox.sh/pyne-vscode.vsix](https://hoox.sh/pyne-vscode.vsix) (unversioned alias). Versioned `pyne-vscode-0.4.4.vsix` is built in-tree (`vscode-extension/`) and on GitHub Releases.
