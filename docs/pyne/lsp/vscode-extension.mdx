...
const serverOptions: ServerOptions = {
  command: lspCommand,           // default pyne-lsp
...
`scriptPath` is resolved to `../src/pynescript/langserver` relative to the extension for monorepo development. Release packaging typically expects `pyne-lsp` (or `pynescript-lsp`) on `PATH` (pip or Nuitka binary).
