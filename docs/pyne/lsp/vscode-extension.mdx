...
No `--parent-dir` / `PYTHONPATH` rewrite. Transport is STDIO. Release packaging expects `pyne-lsp` on `PATH` (pip, Nuitka onefile, or `docker run -i ghcr.io/hoox-sh/pyne/lsp:0.3.12`).
...
npm run package          # pyne-vscode-0.3.12.vsix
# or monorepo:
make build-vscode
```
