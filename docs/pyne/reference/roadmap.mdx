...
---
title: "Roadmap"
description: "PYNE forward plan — dual-host residual, corpus ~94.3%, plot parity harness; H2/T1/F2/L2 landed (2026-08)."
---
...
The roadmap is a **living prioritization document**, not a contract. Source: `docs/ROADMAP.md` (updated **2026-08-03**). Core v6 language/builtins are essentially closed. Product **warm-compile (H2)**, **series caps (T1)**, **pending-fill (F2)**, and **L2 webhooks** have **landed**. What remains: **dual-host package-level Runtime unify (H1 residual)**, **corpus execution tail (C1 ~94.3%)**, **interpret↔compile plot residuals (P1p harness landed)**, residual nested TA (**T2** partial), and optional **TV-oracle re-baselines (F1)**.
...
| PyPI **`hoox-pyne`** | ✅ live 0.3.0 ([pypi.org/project/hoox-pyne](https://pypi.org/project/hoox-pyne/)) |
...
| `request.security` foreign-na | ✅ same-symbol OHLCV only; foreign → `na` |
...
| Dual-host package Runtime unify | ⬜ open (largest P1 residual) |
...
| **C1** | Corpus TIMEOUT / RUN_FAIL residual (set01–04 **~94.3%** OK projected) | P1 | ⚙️ ongoing |
...
| **T2** | Incremental TA for remaining heavy kernels | P2 | ✅ partial (bb/kama/cmo/stochrsi; nested full paths residual) |
...
| Horizon | Focus |
| --- | --- |
| Short | H1 package unify residual, **P1p** plot MISMATCH goldens, **C1** corpus tail |
...
- **Corpus:** C1 Runtime OK ~**94.3%** projected (set01–04); residual RUN_FAIL mostly intentional `runtime.error` demos, lower-TF security guards, scrape/PARSE stubs.
