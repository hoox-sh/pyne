...
```mermaid
flowchart LR
  DONE[Core parser + evaluator] --> H1[H1 package SoT ✅]
...
| Numba + object-mode compile | ✅ MVP+ (disk IR cache, `mode=auto`, `time_arr`, cache recovery; **0.3.16** UDT/switch/matrix/drawing residuals) |
| Pro API + auth + Docker | ✅ (`mode` default `auto`; targets api / api-dev / lsp / **cli**) |
| PyPI **`hoox-pyne`** | ✅ live **0.3.16** ([pypi.org/project/hoox-pyne](https://pypi.org/project/hoox-pyne/)); CLIs `pyne` / `pyne-lsp` |
...
- Colocated TS extra tool extracted (0.3.7). Product TypeScript library is [PyneTS](/pyne/docs/pynets).
...
| **P1p** | Compile/interpret **plot parity** residual | P1 | ⚙️ first-party goldens landed; residual corpus MISMATCH tail |
...
| **L3** | Legacy TS Worker builtin parity | P3 | sister `hoox-sh/pine-worker` only — not a product-docs target; new TS work is [PyneTS](/pyne/docs/pynets) |
...
| Horizon | Focus |
| --- | --- |
| Short | **P1p** corpus MISMATCH tail; H1 worker extras (logs/profile) |
...
| sister `hoox-sh/pyne-worker` | Python edge evaluate host — [docs](/pyne/docs/pyne-worker) |
| sister `hoox-sh/pyne-agent-worker` | NL authoring host — [docs](/pyne/docs/agent) |
