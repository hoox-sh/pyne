...
P0 docs honesty → P1 dual-host H1 ✅ (package SoT + shims + worker thin wrap)
                ...
                → H2 warm compile ✅ · T1 series caps ✅ · F2 pending-fill ✅ · C1 corpus ✅
                              ↘ T2 leftover nvi/pvi · F1 ATR/supertrend goldens
...
- pine-worker extra tool extracted to sister `hoox-sh/pine-worker` (not colocated; 0.3.7)
...
| **F1** | ATR Wilder / TV supertrend re-baseline only with dedicated goldens | P2 | ⚙️ interpret ATR is Wilder RMA of TR; optional TV supertrend ratchet goldens open |
...
| Horizon | Focus |
| --- | --- |
| Short | **P1p** plot MISMATCH goldens; H1 worker extras (logs/profile) |
...
| sister `hoox-sh/pine-worker` | Legacy TS Cloudflare Worker (not in this checkout) |
| `pynets/` submodule | TS library snapshot (may lag standalone `@hoox-sh/pynets` 0.2.0) |
...
5. Do **not** re-open **H1 / H2 / T1 / F2 / L2 / C1** as greenfield — they are marked ✅ (H1 residual = worker extras only) in `docs/ROADMAP.md` (2026-08-16).
...
### pine-worker maturity gate (sister checkout)

```bash
# not present in this repo — clone hoox-sh/pine-worker
cd ../pine-worker && bun test && bun run typecheck
```

Prefer [PyneTS](/pyne/docs/pynets) (`bun test` in `hoox-sh/pynets`) for new TypeScript library work.
