...
| Dual Runtime host copies (**H1**) | ⚙️ Host surface ported (fail-cache / `error_kind` / inputs-auto / compile cache); package-level unify still open |
| Warm-compile product path (**H2**) | ⬜ SLOs / prewarm / deploy IR cache defaults |
| Corpus Runtime tail (**C1**) | ⚙️ ~**93.7%** set01–04 OK projected (was ~89.8%); PARSE stubs ~118; residual str.replace / timestamp / indices |
