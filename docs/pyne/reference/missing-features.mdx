...
| Alert engine + L2 webhooks | `alert()`/`alertcondition()` freq rules, dual-host `/run` export, HTTP webhooks |
...
| **`request.security` foreign data on compile** | ⚙️ **Not filled** — compile emits `na` for foreign tickers / complex security expressions (no invent of chart-as-foreign series). Same-symbol simple OHLCV may lower. Interpret may still use mocks/feeds. |
...
| Corpus Runtime tail (**C1**) | ⚙️ ~**94.3%** set01–04 OK projected (8-agent pass); PARSE stubs ~118; ~21 residual RUN_FAIL |
| Series cap (**T1**) / residual TA inc (**T2**) | ⬜ Partial — hot path done; BB/nested full paths remain |
| TV Wilder RMA-ATR (**F1**, if re-baselined) | ⚠️ Current oracle uses EMA-of-TR; change is correctness, not silent perf |
| Alert webhooks (**L2**) | ✅ Pro API + pyne-worker (`ALERT_WEBHOOK_URL` / `webhook_url`); see [Alerts](/pyne/docs/runtime/alerts) |
