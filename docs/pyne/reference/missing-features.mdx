...
description: "Known gaps and closed Pine Script™ v6 items — H1/H2/T1/T2-volume/F2/L2/C1 shipped; P1p plot residual and nvi/pvi remain."
---
...
“Missing” in PYNE is a **moving frontier**. As of **2026-08-16** (`hoox-pyne` **0.3.10**), core v6 support remains very high (~99%+ for core language), with post-launch TV items closed (multiline strings, library `export const`, footprint mocks, risk enforcement, matrix LA, compile-mode strategy, drawing GC, …) **plus** open-source corpus parse/Runtime hardening, **incremental bar-mode TA** (including 0.3.10 volume kernels), **alert engine + L2 webhooks**, **series caps**, **warm-compile**, **pending-fill** averaging, and **package Runtime SoT** (`pynescript.runtime`; `backend.runtime` is a shim). Remaining gaps cluster into **plot parity residual MISMATCH (P1p)**, leftover full-list TA (`ta.nvi` / `ta.pvi`), **by-design platform differences**, **editor-only features**, and optional **F1** fidelity goldens.

Authoritative narrative: `docs/missing_features.md`. Canonical IDs: `docs/ROADMAP.md` (**2026-08-16**).
...
flowchart TB
  ...
  subgraph Remaining
    ...
    T2r[T2 leftover nvi / pvi]
    F1[F1 ATR / supertrend goldens]
  end
...
| Incremental `ta.*` (MAs, oscillators, `bb`/`kama`/`stochrsi`, **0.3.10** volume `obv`/`wad`/`cmf`/`klinger`) | Bar mode; `PYNE_TA_INCREMENTAL=0` to disable. Residual full-list: `nvi`/`pvi` |
...
| Residual TA inc (**T2**) | ✅ R7 bb/kama/cmo/stochrsi + wma/hma/linreg; **0.3.10** `obv`/`wad`/`wvad`/`cmf`/`klinger`. Residual full-list: `ta.nvi` / `ta.pvi` |
...
| TV Wilder RMA-ATR / supertrend (**F1**) | ⚙️ interpret ATR is already `ta.rma(ta.tr)`; optional TV supertrend ratchet still needs explicit goldens |
