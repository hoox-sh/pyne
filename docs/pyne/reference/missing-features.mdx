...
“Missing” in PYNE is a **moving frontier**. As of **2026-08-16** (`hoox-pyne` **0.3.11**), core v6 support remains very high (~99%+ for core language), with post-launch TV items closed (multiline strings, library `export const`, footprint mocks, risk enforcement, matrix LA, compile-mode strategy, drawing GC, …) **plus** open-source corpus parse/Runtime hardening, **incremental bar-mode TA** (including 0.3.10–0.3.11 volume kernels), **alert engine + L2 webhooks**, **series caps**, **warm-compile**, **pending-fill** averaging, and **package Runtime SoT** (`pynescript.runtime`; `backend.runtime` is a shim). Remaining gaps cluster into **plot parity residual MISMATCH corpus tail (P1p)**, **by-design platform differences**, and **editor-only features**. Incremental `ta.nvi`/`ta.pvi` and Supertrend mid±factor·ATR goldens have landed.
...
| Residual TA inc (**T2**) | ✅ R7 bb/kama/cmo/stochrsi + wma/hma/linreg; **0.3.10** `obv`/`wad`/`cmf`/`klinger`; **0.3.11** `nvi`/`pvi` |
