...
description: "Known gaps and closed Pine Script™ v6 items — 0.6.7 interpret dispatch and compile ta.uo; 0.6.6 last-sample TA and convert/for-n safety; 0.6.5 convert-to-v6 UDF-safe prefixes; 0.6.4 convert-to-v6; 0.6.3 hosted runner; 0.6.2 P1p official builtins 0 MISMATCH."
---
...
flowchart TB
  ...
  subgraph Remaining
    ...
    ED[Editor-only UX]
    P1p[optional set0x community-corpus plot parity]
  end
...
| PyneTS full builtin parity (**L3**) | ⚙️ interpret + JS compile landed in `@hoox-sh/pynets` 0.2.0; Python remains oracle. Open rows: [PyneTS missing features](/pyne/docs/pynets/missing-features) |
