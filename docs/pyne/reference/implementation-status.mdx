...
| PyneTS | Standalone `@hoox-sh/pynets` **0.2.0** — interpret + JS compile. This repo’s `pynets/` submodule pin is **0.1.0** interpret-only — see [PyneTS](/pyne/docs/pynets) |
| pine-worker | Legacy TS Cloudflare Worker (sister repo; not in this checkout) |
| Numba compile path | MVP + object-mode fallback |
| Incremental `ta.*` | Default on (`PYNE_TA_INCREMENTAL`); **0.3.10** adds `obv`/`wad`/`cmf`/`klinger`. Residual: `nvi`/`pvi` |
