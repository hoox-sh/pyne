...
Source distillations: `docs/compatibility_guarantee.md` and live test inventory. Package **`hoox-pyne` 0.3.10**. Metrics move as the suite grows — prefer CI and the [implementation status](/pyne/docs/reference/implementation-status) matrix for current checkmarks. Compatibility is **not** bit-identical TradingView® platform parity and is **not** a TradingView® platform-identity claim.
...
| --- | --- | --- |
| Parser & syntax | Full grammar on first-party fixtures and unit snippets | Solid |
| AST round-trip | Structural match on test suite | Verified |
| Builtins | Broad coverage (200+ / dispatch hundreds) | Good → excellent by namespace |
| Technical indicators | Core + advanced; numerical cross-checks | Validated in tests |
...
| Open corpus set01–04 (local) | Parse **99.96%**; Runtime interpret **100%** excl. intentional demos (2026-08-09) | Measured — not TV platform parity |
| Third-party corpora | **Not shipped** in-repo (legal / ToS hygiene) | N/A |

### Corpus rates (set01–04 · 2477 scripts · 2026-08-09)

| Suite | Rate | Notes |
| --- | ---: | --- |
| Parse + unparse | **99.96%** (2476/2477) | Residual: intentional invalid line-wrap docs demo |
| Runtime interpret (50 bars) | **100%** excl. EXPECTED_FAIL | 2466 OK + 11 intentional demos |
| set01 Runtime | **249/249 (100%)** | Both interpret and prior compile sweeps |

**EXPECTED_FAIL** paths are path-listed intentional demos (`runtime.error` library guards, lower-TF security, pathological loops) — never unknown soft-suppression. Do not market “full TV parity.” Re-run `make test` for live suite counts.
...
1. **Foreign `request.*` / foreign-na** — compile does **not** invent multi-asset series. Same-symbol simple OHLCV may lower; foreign tickers and complex security expressions emit `na` on both backends when data is absent (interpret may still serve mocks/feeds if wired). Prefer honest NA over chart-close-as-foreign. See [Missing features](/pyne/docs/reference/missing-features).
...
4. **Structural plot keys** — titled `fill()` / constant `hline` key sets may differ by design; harness flags `--ignore-hline-keys` / `--ignore-fill-keys` isolate value residuals from export shape.
...
| `tests/fixtures/parity/` | First-party strategy parity fixtures |
| `tests/test_parse_and_unparse.py` | Round-trip (optional local `--example-scripts-dir`) |
| `scripts/compare_interp_compile.py` | Interpret ↔ compile series/plot parity harness |
| `tests/test_interp_compile_parity.py` | Parity smoke |
...
### Optional local example-script sweep

```bash
# Only if you provide your own *.pine directory (not shipped)
pytest tests/test_parse_and_unparse.py -q --example-scripts-dir=/path/to/your/scripts
```
