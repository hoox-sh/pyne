...
description: "Grouped map of what PYNE implements for Pine Script™ v5/v6 — working, partial, residual, and by-design out of scope. hoox-pyne 0.6.2."
---
...
PYNE implements Pine Script™ **v5/v6 language core** as an inspectable pipeline: parse → AST → bar-loop evaluate. The map below is the product-facing compatibility surface for **`hoox-pyne` 0.6.2**. It is **not** TradingView® platform identity (chart host, proprietary data, editor-only UI) and **not** bit-identical results vs the hosted platform on every script or bar.
...
<AccordionGroup>
  <Accordion title="Language — working">
    ...
    | Control flow, `switch`, `once`, enums | Working | Strict bool; `na` is false in conditions. `once` (0.4.4) is statement-only |
    ...
    | `timeframe.change` | Working | Intraday fixed-width buckets; D/W/M on the exchange calendar (0.6.0, `syminfo.timezone`, UTC default) |
  </Accordion>
  ...
  <Accordion title="Bar-loop host — working">
    ...
    | Incremental `ta.*` | Working | Default **on**; `PYNE_TA_INCREMENTAL=0` full recompute. **0.3.12** `aroon`/`dpo`/`donchian`/`kst` |
  ...
  <Accordion title="ta.* — working">
    ...
    | Oscillators | Working | `rsi`/`macd`/`stoch`/`cci`/`cmo`/`tsi`/`roc`/`wpr`; **0.3.12** incremental `aroon`/`dpo`/`kst` |
    | Volatility / structure | Working | `atr` (Wilder `rma(tr)`), `bb`, `kc`, `stdev`, `highest`/`lowest`, `linreg`, `adx`/`dmi`, `supertrend`, `sar`; **0.3.12** incremental `donchian` |
  ...
  <Accordion title="Drawing and plots — working; renderer external">
    | Area | Status | Notes |
    | --- | --- | --- |
    | `plot` / `plotshape` / `plotchar` / `plotarrow` / `hline` / `bgcolor` / `barcolor` / `fill` / `plotbar` / `plotcandle` | Working | Registry + columnar capture; plotshape off = `None` (dual-host) |
    | `line` / `box` / `label` / `table` / `polyline` / `linefill` | Working | Coordinates snapshotted at create/set; compile `__drawings` normalized to geometry-only AXIS export |
  ...
  <Accordion title="request.* and data — partial by design">
    | Area | Status | Notes |
    | --- | --- | --- |
    | Same-symbol simple OHLCV / HTF | Working | Allowlisted `ta.sma`/`ema`/`rsi`/`atr`/`wma`/`rma` on HTF |
  ...
  <Accordion title="Alerts, compile, hosts">
    ...
    | Warm compile / disk IR | Working | Prewarm CLI/API; cache meta v16 |
    | Interp ↔ compile plot parity | Working | Harness + first-party goldens + official TV builtins **0 MISMATCH** (P1p); optional set0x community corpus unmeasured |
    ...
    | `timeframe.change` | Working | Intraday fixed-width; D/W/M exchange calendar. Interpret + compile (calendar via object mode) |
    | `POST /optimize` / `pyne optimize` | Working | Strategy `input.*` search — not a Pine builtin |
  </Accordion>
  ...
  <Accordion title="Sister implementations">
    ...
    | pyne-worker | Working | Thin Python CF host; vendors engine (may lag 0.3.16). Docs: [pyne-worker](/pyne/docs/pyne-worker) |
    | pyne-agent-worker | Working | NL authoring; optional `/run` validate. Docs: [agent](/pyne/docs/agent) |
  </Accordion>
</AccordionGroup>
