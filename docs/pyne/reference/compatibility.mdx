...
Source distillations: `docs/compatibility_guarantee.md` (v1.2, 2026-08-03) and live test inventory. Metrics move as the suite grows — prefer CI and the [implementation status](/pyne/docs/reference/implementation-status) matrix for current checkmarks. Corpus Runtime set01–04 ~**94.3%** OK is an honest execution rate on open scripts, **not** bit-identical TradingView® platform parity.
...
| Real-world scripts | High parse success; Runtime OK **~94.3%** on set01–04 | Measured / residual tracked |

**Corpus honesty (set01–04 scraped corpus):** after sanitize, parse/unparse is ~**99.96%** OK (one intentional invalid line-wrap demo remains FAIL). **Runtime OK ~94.3%** on the same set—not 100%. Residual RUN_FAIL classes include intentional `runtime.error` library demos, foreign `request.*` without multi-symbol feeds, period edges, and still-missing surface. Prefer re-running showcase/corpus tooling for live percentages; do not market “full TV parity.”
