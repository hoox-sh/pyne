...
Decorated with `@require_admin_token`. **Implementation note:** `require_admin_token` currently sets `g.is_admin = True` unconditionally (placeholder); production deployments should enforce `ADMIN_TOKEN` / role checks before exposing this route. App docstring describes `X-Admin-Token` + `ADMIN_TOKEN` env as the intended design.
...
| `require_admin_token` | Placeholder admin gate for create_key |
...
| Module | Use |
| --- | --- |
| `middleware/auth.py` → `APIKeyStore` | Default JSON file store (`API_KEY_STORE` path) |
| `middleware/key_store_sqlite.py` | Hash-only rows, WAL, multi-worker friendly |
| `middleware/key_store_redis.py` | Redis-backed alternative |

Default `get_key_store()` returns the JSON-file `APIKeyStore` singleton. JSON store currently keeps raw keys as dict keys in the file (dev convenience); SQLite module documents **hash-only** storage for safer leaks.
...
5. **Admin decorator gap** — do not assume create_key is locked in all deployments until the TODO is closed.

## Worked example

```bash
# create (when admin gate is open / local tests)
