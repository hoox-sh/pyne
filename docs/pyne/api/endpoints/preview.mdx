...
| code | When |
| --- | --- |
| `NO_DATA` | Missing `data` (both preview routes) |
| `NO_CLOSE_DATA` | Empty `close` array (chart **and** indicator) |
| `RENDER_ERROR` | matplotlib / renderer exception (`/preview/chart` only; indicator parse/render exceptions fall back to plotting `close`) |
