...
| `libraries` | list | no | `[]` | `[{namespace, name, version, source}]` — AXIS git-publish emulator (0.3.7+; max 32) |

Unknown fields → `UNKNOWN_FIELDS` 400. Empty script/data still checked after schema with `NO_SCRIPT` / `NO_DATA`. `import` scripts are compile-ineligible; `mode=auto` falls back to interpret and **must** keep `libraries` (0.3.8+).
...
7. Structured errors may include `error_kind` (`parse|compile|runtime|data|order|mode`), `error_type`, `error_bar`; timeout runs may set `timed_out` when the host passes `timeout_seconds`.
