...
- Free compute also gated by `backend/middleware/free_limits.py`: max bars (`FREE_MAX_BARS`, default 5000), script chars (`FREE_MAX_SCRIPT_CHARS`, 256 KiB), per-IP rate (`FREE_RATE_LIMIT` / `FREE_RATE_WINDOW_SEC`, 60 / 60s), concurrency (`FREE_MAX_CONCURRENT`, 4). Chart/`mock` data sources only.
...
| `backend/middleware/free_limits.py` | Bar / script / rate / concurrency caps |
