...
| `ADMIN_TOKEN` | unset | Fail-closed admin minting (see [auth](/pyne/docs/api/auth-and-keys)). When set, hosted-runner writes also require it |
| `PYNE_RUNNER` | off | Optional hosted script registry + `/cron/run` ([runner](/pyne/docs/api/endpoints/runner)) |
| `PYNE_RUNNER_SCHEDULER` | off | In-process bar-close poll (requires `PYNE_RUNNER`) |
| `PYNE_RUNNER_POLL_SECONDS` | `60` | Scheduler interval |
| `PYNE_RUNNER_DB` | `/data/runner.db` or temp | SQLite registry path |
