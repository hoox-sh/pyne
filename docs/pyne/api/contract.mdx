...
The **evaluate contract** is the JSON dialect that lets AXIS, HOOX, CLI tools, and edge workers call *any* PYNE-compatible host interchangeably. Flask `POST /run` is the reference implementation; Python **[pyne-worker](/pyne/docs/pyne-worker)** aims for the same HTTP fields. **[PyneTS](/pyne/docs/pynets)** speaks the in-process form (`RuntimeResult`).
