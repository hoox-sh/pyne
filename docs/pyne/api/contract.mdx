...
| `mode` | `"interpret"` \| `"compile"` \| `"auto"` | no | Desk default interpret; Pro API schema default `auto` |
| `inputs` | object | no | `input.*` overrides keyed by title (forces interpret under `auto`) |
| `libraries` | `{namespace, name, version, source}[]` | no | In-process `import ns/Name/ver` (0.3.7+; max 32; interpret/`auto`) |
| `profiler` | bool | no | Per-line interpret timings |
| `timeout_seconds` | number | no | Wall-clock interpret budget (edge hosts cap, e.g. 30s) |
