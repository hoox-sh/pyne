...
The Python distribution name on PyPI is **`hoox-pyne`**. After install, the import path and CLIs stay **`pynescript`** / **`pynescript-lsp`**.
...
#    src/pynescript/__about__.py  →  e.g. 0.3.1
#    CHANGELOG.md
...
python -m build
twine check dist/*
# expect: hoox_pyne-*.whl  and  hoox_pyne-*.tar.gz
...
If tag `v0.3.0` already exists and the wheel was never uploaded, re-run:
