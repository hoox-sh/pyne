...
The Python distribution name on PyPI is **`hoox-pyne`**. After install, the import path and CLIs stay **`pynescript`** / **`pynescript-lsp`**. Automation lives in `.github/workflows/publish.yml` and uses [Trusted Publishing](https://docs.pypi.org/trusted-publishers/) (OIDC) against the GitHub environment **`pypi`**.
