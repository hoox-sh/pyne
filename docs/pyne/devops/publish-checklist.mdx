...
description: "Transfer pyne to hoox-sh, wire PyPI Trusted Publishing, then cut v0.3.0."
---
...
First public ship of **`pyne` 0.3.0** should happen **after** the GitHub
repo is under the **`hoox-sh`** org. Trusted Publishing binds to
`owner/repo/workflow/environment` — register (or re-register) the publisher
against **`hoox-sh/pyne`**, not a personal account.

## 0. Identity map

| Surface | Value |
| --- | --- |
| GitHub org | [`hoox-sh`](https://github.com/hoox-sh) (display: hoox.sh) |
| Repo | [`hoox-sh/pyne`](https://github.com/hoox-sh/pyne) |
| PyPI project | `hoox-pyne` |
| Import / CLIs | `pynescript`, `pynescript-lsp` |
| Package version | `src/pynescript/__about__.py` → `0.3.0` |
| VS Marketplace publisher | `jango-blockchained` (unchanged; Marketplace id ≠ GitHub org) |
...
## 2. PyPI Trusted Publishing (must match org)

On [pypi.org](https://pypi.org) → Account → **Publishing** → **Add a new pending publisher**:
...
If you previously added a pending publisher for `jango-blockchained`, **delete it** and create one for `hoox-sh` before the first upload.
