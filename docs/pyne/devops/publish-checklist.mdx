...
| Package version | `src/pynescript/__about__.py` → **0.3.12** |
| VS Marketplace / Open VSX publisher | `hoox-sh` (`hoox-sh.pyne` 0.3.12) |
...
```bash
# version already 0.3.12 in src/pynescript/__about__.py
pip install build twine
rm -rf dist/
python -m build
twine check dist/*
# expect: hoox_pyne-0.3.12-py3-none-any.whl  hoox_pyne-0.3.12.tar.gz
...
1. Move `CHANGELOG.md` **Unreleased** notes into `## [0.3.12] - YYYY-MM-DD`.
2. Align VSIX version in `vscode-extension/package.json` (already **0.3.12**) if shipping Marketplace/VSIX with this tag.
3. Tag and push:

```bash
git tag -a v0.3.12 -m "v0.3.12"
...
4. Watch in parallel:
   ...
   - **GHCR** → `ghcr.io/hoox-sh/pyne/{api,cli,lsp}:0.3.12`

## 6. Post-publish verify

```bash
pip index versions hoox-pyne   # or: pip install hoox-pyne==0.3.12
python -c "import pynescript; print(pynescript.__version__)"
# GitHub Release: https://github.com/hoox-sh/pyne/releases/tag/v0.3.12
# PyPI: https://pypi.org/project/hoox-pyne/0.3.12/
```
