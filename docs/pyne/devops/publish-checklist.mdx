...
| Package version | `src/pynescript/__about__.py` → **0.6.3** |
...
```bash
# version already 0.6.3 in src/pynescript/__about__.py
pip install build twine
rm -rf dist/
python -m build
twine check dist/*
# expect: hoox_pyne-0.6.3-py3-none-any.whl  hoox_pyne-0.6.3.tar.gz
...
1. Move `CHANGELOG.md` **Unreleased** notes into `## [0.6.3] - YYYY-MM-DD`.
...
4. Watch in parallel:
   ...
   - **GHCR** → `ghcr.io/hoox-sh/pyne/{api,cli,lsp}:0.6.3`

## 6. Post-publish verify

```bash
pip index versions hoox-pyne   # or: pip install hoox-pyne==0.6.3
python -c "import pynescript; print(pynescript.__version__)"
# GitHub Release: https://github.com/hoox-sh/pyne/releases/tag/v0.6.3
# PyPI: https://pypi.org/project/hoox-pyne/0.6.3/
```
