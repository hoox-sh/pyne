...
REGISTRY=ghcr.io/hoox-sh/pyne TAG=0.5.0 docker buildx bake release
# release group = api-release + cli-release + lsp-release
# → ghcr.io/hoox-sh/pyne/{api,cli,lsp}:0.5.0
...
```
ghcr.io/hoox-sh/pyne/api:0.5.0
ghcr.io/hoox-sh/pyne/cli:0.5.0
ghcr.io/hoox-sh/pyne/lsp:0.5.0
```
...
Substitution `_PYNESCRIPT_VERSION` in that YAML currently defaults to `"0.3.0"` — override at submit time to match `__about__.py` (**0.5.0**).
