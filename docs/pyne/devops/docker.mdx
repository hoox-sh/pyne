...
make docker-up-full    # + redis + lsp profiles
...
| `lsp` | `lsp` | stdio; prefer `docker compose run --rm lsp` |

Network name: `pynescript-dev`.

Production overlay (`docker-compose.prod.yml`): target `api`, no source mounts, mem/cpu caps, requires `ADMIN_TOKEN`.
...
| `API_KEY_STORE` | `/data/api_keys.json` | JSON key store path (writable volume) |
| `ALLOWED_ORIGINS` | see `.env.example` | CORS allow-list |
| `ADMIN_TOKEN` | unset | Required for admin key minting / prod compose |
| `REDIS_URL` | `redis://redis:6379/0` | Optional Redis (when profile active) |
| `GUNICORN_WORKERS` / `THREADS` / `TIMEOUT` | `2` / `4` / `60` | Prod entrypoint knobs |
