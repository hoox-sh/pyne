...
PYNE is a **multi-surface product**: pure-Python library + Click CLI, pygls Language Server, Flask Pro API, VS Code extension, Docker targets (`api` / `cli` / `lsp`), and colocated edge workers. The AXIS charting PWA is a **sister repo** — not built in this tree.
...
**Invariant:** CI green on `main` is necessary but not sufficient for a full release. Tag workflows build CLI/LSP binaries + VSIX; `publish.yml` uploads **`hoox-pyne`** to PyPI; Cloud Build deploys the API image separately.
...
| Versioned CLI + LSP + VSIX | `.github/workflows/release.yml` | [Release](/pyne/docs/devops/release) |
| PyPI (`hoox-pyne`) | `.github/workflows/publish.yml` | [PyPI publish](/pyne/docs/devops/pypi-publish) |
