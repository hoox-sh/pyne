...
# API in Docker (production target via buildx bake)
make docker-build
make docker-up
make docker-run
```
...
| Cloud Run 502 after deploy | Image missing deps / gunicorn bind | Confirm `Dockerfile` target `api` CMD and `PORT=8080` |
