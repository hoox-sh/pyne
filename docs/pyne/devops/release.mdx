...
Assets attach to a GitHub Release; PyPI upload is handled by `publish.yml` (Trusted Publishing). Marketplace publish is optional via `VSCE_PAT`.
## Conceptual model
...
| `release` | needs package + vscode success; tag push only | GitHub Release body + all staged assets |
...
```bash
# LSP
pip install -e ".[lsp]" "nuitka==2.0.*" cryptography
...
| `pyne-*.vsix` | `code --install-extension pyne-*.vsix` |
