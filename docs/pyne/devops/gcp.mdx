...
| `_PYNESCRIPT_VERSION` | `"0.3.0"` (stale default in YAML) | OCI label; override to **0.3.13** at submit |
...
| `--timeout` | 60s | Platform kill. Entrypoint gunicorn default is **120s** (`GUNICORN_TIMEOUT`); set the env to **60** on Cloud Run or long evals die at the platform first. |
