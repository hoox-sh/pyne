...
The **desk path** is the `pynescript` Click CLI and `pynescript.ast` library API. The **editor path** is `pynescript-lsp` (stdio). The **HTTP path** is the Flask Pro API (`POST /run`, preview, backtest). The optional chart is [AXIS](/axis/docs); the optional trade mesh is [HOOX](/docs). Evaluation does not require either.
...
- **[Installation](/pyne/docs/enduser/getting-started/installation)** — PyPI, extras (`lsp`, `data`), Hatch checkout, smoke checks.
- **[Quick start](/pyne/docs/enduser/getting-started/quick-start)** — Parse → dump → unparse → lint → `literal_eval` in under ten minutes.
- **[Configuration](/pyne/docs/enduser/getting-started/configuration)** — Extras, console scripts, Pro API env vars, editor settings.
...
- **[Evaluate scripts](/pyne/docs/enduser/guides/evaluate-scripts)** — Expression eval, bar-loop Runtime, strategy events, mock vs live data.
- **[Editors](/pyne/docs/enduser/guides/editors)** — VS Code, Neovim, Zed, Emacs, Helix; `clients/` snippets.
- **[Pro API usage](/pyne/docs/enduser/guides/pro-api-usage)** — `/run`, `/run/batch`, preview, backtest as a **consumer**.
...
| --- | --- | --- |
| Install core | `pip install pynescript` | [Installation](/pyne/docs/enduser/getting-started/installation) |
| Install LSP | `pip install "pynescript[lsp]"` | [Editors](/pyne/docs/enduser/guides/editors) |
| Install market data deps | `pip install "pynescript[data]"` | [CLI data](/pyne/docs/enduser/guides/cli) |
| Parse file | `pynescript parse-and-dump path.pine` | [CLI](/pyne/docs/enduser/guides/cli) |
