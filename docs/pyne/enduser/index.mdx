...
| --- | --- | --- |
| CLI | `pyne …` (alias `pynescript`) | One-shot parse, format, lint, compile, `run` (synthetic bars), prewarm, data |
...
- **[CLI](/pyne/docs/enduser/guides/cli)** — `check`, `format`, `lint`, `compile`, `prewarm`, `run`, `data`, …
...
| Path | Role |
| --- | --- |
| `src/pynescript/__main__.py` | Click CLI: check, format, lint, compile, prewarm, run, data, info |
