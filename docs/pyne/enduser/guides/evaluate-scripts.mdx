...
`timeout_seconds` is a **library** argument only. `POST /run` accepts `libraries` / `inputs` / `mode` but **rejects** `timeout_seconds` (`UNKNOWN_FIELDS`). `pyne run` does not call `Runtime.run` at all.
...
| `UNKNOWN_FIELDS` if you POST `timeout_seconds` | Library-only kwarg — not in `RUN_SCHEMA` |
