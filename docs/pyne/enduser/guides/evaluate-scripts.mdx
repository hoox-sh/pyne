...
`backend.runtime.Runtime.run` accepts a `mode` argument that selects how the bar loop executes:
...
| `backend/runtime.py` | Multi-bar `Runtime.run` (`interpret` / `compile` / `auto`) |
