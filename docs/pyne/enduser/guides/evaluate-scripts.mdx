...
description: "Run Pine expressions and multi-bar scripts with literal_eval, pynescript.runtime.Runtime (libraries=, timeout_seconds, mode), data providers, and strategy events."
---
...
| `src/pynescript/runtime/host.py` | Multi-bar `Runtime.run` SoT (`interpret` / `compile` / `auto`, `timeout_seconds`) plus `InterpretSession` (`create_session` / `append_bars` / `update_last_bar`) |
...
| `timed_out` + partial plots | `timeout_seconds` budget exceeded on interpret |
| `create_session supports interpret only` | `mode="compile"` / `"auto"` passed to `create_session` — sessions are interpret-only |
| `UNKNOWN_FIELDS` | Extra key not in `RUN_SCHEMA` / `RUN_BATCH_SCHEMA` |
