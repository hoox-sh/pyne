...
3. **`pynescript.runtime.Runtime`** (package SoT; `backend.runtime` re-exports) or **Pro API `POST /run`** — bar-loop over OHLCV with shared evaluate contract  
...
# optional wall-clock budget (0.3.4+ / unreleased hosts):
