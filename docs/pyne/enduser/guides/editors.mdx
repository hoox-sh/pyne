...
**Marketplace / VSIX:** extension id **`hoox-sh.pyne`** (publisher `hoox-sh`, package name `pyne`). [VS Marketplace](https://marketplace.visualstudio.com/items?itemName=hoox-sh.pyne). Local package: `cd vscode-extension && npm ci && npm run package` then `code --install-extension pyne-vscode-0.3.10.vsix`. The VSIX is self-contained; you still need `pip install "hoox-pyne[lsp]"` (or a `pyne-lsp` binary) on the machine.
