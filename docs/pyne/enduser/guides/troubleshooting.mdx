...
| Eval | `src/pynescript/ast/evaluator/`, `src/pynescript/runtime/` |
...
If minimal works but strategy fails: remove `request.*`, drawings, then strategy entries until isolated. Check `mode`; try `"interpret"`. Library `Runtime.run` defaults to **interpret**; `POST /run` defaults to **auto**; `pyne run` is compile-only synthetic smoke — they will not match each other unless you set `mode` explicitly. See [modes](/pyne/docs/enduser/reference/modes).
...
| `pyne run` ≠ `Runtime.run` | modes | CLI is compile-only synthetic; library default is interpret |
| `UNKNOWN_FIELDS` + `timeout_seconds` | HTTP | field is valid on `/run` and `/run/batch` (0.3.11+); other extra keys still 400 |
