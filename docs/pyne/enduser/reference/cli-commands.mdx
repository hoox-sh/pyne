...
description: "Complete option reference for the pyne Click CLI: check, format, lint, compile, run, optimize, prewarm, data, info, and more."
---
...
(e.g. `0.6.7`). Global `--no-color` honors `NO_COLOR`.
...
Library: `pynescript.optimize.run_study`. HTTP: [POST /optimize](/pyne/docs/api/endpoints/optimize).
...
Full HTTP contract: [hosted runner](/pyne/docs/api/endpoints/runner).
