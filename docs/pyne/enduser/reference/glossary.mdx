...
4. **Ecosystem** — AXIS, HOOX, PyneTS, pyne-worker, pine-worker  
...
TypeScript / Bun library (`@hoox-sh/pynets`). Public names match Python: `parse`, `unparse`, `Runtime.run`. Not a Worker. See [PyneTS](/pyne/docs/pynets).

### pine-worker

Legacy TypeScript Cloudflare Worker in a sister repository. **Not** colocated under `pynescript/pine-worker/`. See [pine-worker](/pyne/docs/pine-worker).

### pyne-worker

Python Cloudflare Worker (sister repo) embedding pynescript for edge `/run`. See [HOOX isolate](/docs/devops/workers/pyne-worker).
...
Optional `Runtime.run` wall-clock budget (interpret path). Exceeded runs return partial plots with `timed_out`. **Not** a `POST /run` body field.
