...
- **`hoox-pyne`** — **PyPI distribution** name ([live](https://pypi.org/project/hoox-pyne/)). Install with `pip install hoox-pyne`. Do **not** install plain `pyne`/`PyNE` (unrelated) or the upstream `pynescript` package (elbakramer).
- **`pynescript`** — **import package** and CLI name (`import pynescript`, `pynescript` / `pynescript-lsp` scripts). Repo product name is also often called pyne.
