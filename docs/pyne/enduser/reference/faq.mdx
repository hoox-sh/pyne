...
- **`hoox-pyne`** — **PyPI distribution** name ([live](https://pypi.org/project/hoox-pyne/), **0.3.4+**). Install with `pip install hoox-pyne`. Do **not** install plain `pyne`/`PyNE` from PyPI without the `hoox-` prefix (unrelated) or the upstream `pynescript` package (elbakramer).
