...
- **`hoox-pyne`** — **PyPI distribution** name ([live](https://pypi.org/project/hoox-pyne/), **0.6.7**). Install with `pip install hoox-pyne`. Do **not** install plain `pyne`/`PyNE` from PyPI without the `hoox-` prefix (unrelated) or the upstream `pynescript` package (elbakramer).
...
Yes, as of **hoox-pyne 0.4.4** (TradingView® August 2026). `once [cond]` runs its block the first time the condition is true on a closed bar. It is **not** an expression — you cannot assign its result. See [Expressions & statements](/pyne/docs/runtime/expressions-statements).
...
See [Interpret ↔ compile parity](/pyne/docs/runtime/compiler/parity). Pro API `POST /run` defaults to `mode=auto`. The library defaults to `interpret`. Force `interpret` when you need alerts, `input.*` overrides, or full `request.*`.
...
- **[pyne-worker](/pyne/docs/pyne-worker)** — Python Cloudflare Worker. Production `POST /run`. Vendors `pynescript.runtime`.
- **[pyne-agent-worker](/pyne/docs/agent)** — NL authoring (`POST /v1/chat`). Standalone chat; optional generate → `/run` validate.

The name **pine-worker** is a leftover TypeScript experiment ([hoox-sh/pine-worker](https://github.com/hoox-sh/pine-worker)). It is not colocated here and is not a product docs target.
