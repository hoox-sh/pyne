...
---
title: "Quick Start"
description: "Parse, dump, unparse, lint, and evaluate Pine Script™ with the pyne CLI and library in under ten minutes."
---
...
```bash
pip install hoox-pyne                 # https://pypi.org/project/hoox-pyne/ · 0.3.10+
# optional:
...
Omit `mode` to use the schema default **`auto`** (warm compile when eligible, else interpret). Response may include `series`, `plot_meta`, `events`, `drawings`, and `alerts`. See [Pro API usage](/pyne/docs/enduser/guides/pro-api-usage) for batch mode, webhooks, free-tier guards, and prewarm.
