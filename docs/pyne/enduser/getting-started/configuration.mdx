...
Free-tier Pro API guards (unauthenticated `/run`): `FREE_MAX_BARS`, `FREE_MAX_SCRIPT_CHARS`, `FREE_MAX_CONCURRENT`, `FREE_RATE_LIMIT` / `FREE_RATE_WINDOW_SEC` — see [Pro API usage](/pyne/docs/enduser/guides/pro-api-usage).
...
| `pynescript.lsp.command` | `pyne-lsp` | Binary path (alias `pynescript-lsp` also works) |
