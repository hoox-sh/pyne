...
| `libraries` | list | `[{namespace, name, version, source}]` — max 32; interpret / `auto` fallback |
...
`timeout_seconds` is a **library** `Runtime.run(...)` argument, not a `POST /run` body field (unknown keys → `UNKNOWN_FIELDS`).
