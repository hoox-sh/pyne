...
---
title: "Installation"
description: "Install the pyne package from PyPI or a source checkout, select extras for LSP and market data, and verify the CLI and library imports."
---
...
PYNE ships on PyPI as **`pyne`**. The **import package** and console scripts remain **`pynescript`** / **`pynescript-lsp`** (src-layout under `src/pynescript/`). Do **not** install `hoox-pyne` or the unrelated upstream `pynescript` package when you want this stack.

The default install gives you the Click CLI (`pynescript`) and the AST library (`parse`, `unparse`, `dump`, `walk`, `literal_eval`, linter). Optional extras add the Language Server (`pynescript-lsp`) and CCXT-backed market data. A Hatch-managed checkout is the supported path for contributors who need tests, grammar tooling, or the Flask Pro API in-tree.

## Conceptual model

```mermaid
flowchart LR
  PYPI[PyPI pyne] --> CORE[Core wheel]
...
| --- | --- | --- |
| `pip install pyne` | CLI + library | Desk use, scripts, CI lint |
| `pip install "pyne[lsp]"` | + pygls LSP server | Editors |
| `pip install "pyne[data]"` | + ccxt | Live/historical exchange data |
| `pip install "pyne[pro]"` | + Flask Pro API stack (when published) | Self-hosted `/run` |
...
- **Python ≥ 3.10** (classifiers declare 3.10–3.13; pin to 3.10+ for production)
- `pip` or an equivalent installer; virtualenv strongly recommended
- For Pro API locally: `flask`, `flask-cors`, `numpy`, `matplotlib` (see `backend/requirements.txt`) or `pip install "pyne[pro]"` when that extra is available
- For VS Code extension build: Node 22+ under `vscode-extension/` (optional)

### Core install (PyPI / local package)

From a release or built wheel (**distribution name `pyne`**):
...
# alias:
pip install "pyne[datafeed]"

# LSP + pytest-lsp for protocol tests (source checkout)
...
- **PyPI name ≠ import name.** Install **`pyne`**; `import pynescript`. Upstream PyPI `pynescript` (elbakramer) is a different project.
...
- **LSP is not a CLI subcommand of `pynescript`.** The registered script is **`pynescript-lsp`** (`python -m pynescript.langserver`).
- **Pro API.** Prefer monorepo `pip install -r backend/requirements.txt` + `make run`, or the `[pro]` extra when published. Local bind: `HOST`/`PORT` (default `127.0.0.1:5002`).
- **Source extensions.** HOOX / PYNE tooling prefers **`.pyne`** for stack sources; `.pine` / `.pinev5` / `.pinev6` still work (VS Code extension registers all).
...
| `No module named 'pygls'` | LSP import without extra | `pip install "pyne[lsp]"` |
| `No module named 'ccxt'` | data CLI/provider path | `pip install "pyne[data]"` |
| Wrong package / missing APIs | Installed upstream `pynescript` or `hoox-pyne` | `pip uninstall …` then `pip install pyne` |
