...
**PYNE is live on PyPI** as **[`hoox-pyne`](https://pypi.org/project/hoox-pyne/)** (currently **0.3.0**). The **import package** and console scripts remain **`pynescript`** / **`pynescript-lsp`** (src-layout under `src/pynescript/`).

Do **not** install:

- plain **`pyne` / `PyNE`** — unrelated 0.1.0 process-networking library
...
| --- | --- | --- |
| `pip install hoox-pyne` | CLI + library | Desk use, scripts, CI lint |
| `pip install "hoox-pyne[lsp]"` | + pygls LSP server | Editors |
| `pip install "hoox-pyne[compile]"` | + numpy / numba | `compile` / `run` Numba modes |
...
| Docker `pynescript-cli` | Full CLI image | CI gates / isolated runs |
...
| Command | Purpose |
| --- | --- |
| `pynescript` | Click CLI (check, format, lint, compile, run, …) |
...
- **PyPI name ≠ import name.** Install **`hoox-pyne`**; `import pynescript`.
- **License.** AGPL-3.0-or-later. Network use triggers AGPL source-offer obligations (see GNU AGPL §13).
- **LSP is not a CLI subcommand.** Use **`pynescript-lsp`** (or `python -m pynescript.langserver`).
...
| `pynescript` not found | Scripts not on PATH | `python -m pynescript` or fix venv `bin/` |
