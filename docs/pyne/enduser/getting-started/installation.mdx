...
| `pip install "hoox-pyne[data]"` | + ccxt | Live/historical exchange data |
| `pip install "hoox-pyne[pro]"` | + Flask Pro API stack (when published) | Self-hosted `/run` |
...
- For Pro API locally: `flask`, `flask-cors`, `numpy`, `matplotlib` (see `backend/requirements.txt`) or `pip install "hoox-pyne[pro]"` when that extra is available
...
# alias:
pip install "hoox-pyne[datafeed]"
...
```bash
pip install "hoox-pyne[lsp,data]"
# monorepo:
...
| `No module named 'pygls'` | LSP import without extra | `pip install "hoox-pyne[lsp]"` |
| `No module named 'ccxt'` | data CLI/provider path | `pip install "hoox-pyne[data]"` |
