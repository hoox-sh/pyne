...
**PYNE is live on PyPI** as **[`hoox-pyne`](https://pypi.org/project/hoox-pyne/)** (currently **0.3.11+**). The **import package** remains **`pynescript`** (src-layout under `src/pynescript/`). Preferred console scripts are **`pyne`** / **`pyne-lsp`**; legacy **`pynescript`** / **`pynescript-lsp`** remain installed as aliases.
