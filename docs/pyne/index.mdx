...
description: "Open-source Pine Script™ evaluation — grammar, AST, interpret/compile runtime, alerts + webhooks, LSP, Pro API. pip install pyne · github.com/hoox-sh/pyne"
---
...
Install from PyPI as **`pyne`** (`import pynescript`); source lives at [github.com/hoox-sh/pyne](https://github.com/hoox-sh/pyne).

```bash
pip install "pyne[lsp]"
```

The same pipeline powers the desk CLI, the LSP binary, the Pro API, browser Pyodide (via AXIS), and Cloudflare® workers. Round-trip fidelity (`parse → unparse`), interpret↔compile [plot parity](/pyne/docs/runtime/compiler/parity), and open-source corpus Runtime **~94.3%** OK (set01–04 projected) are first-class invariants — see [compatibility](/pyne/docs/reference/compatibility) and [implementation status](/pyne/docs/reference/implementation-status).
...
Install the package (`pip install pyne`), run the CLI, embed the library API, wire an editor (`.pyne` / `.pine`), or call the Pro API as a consumer — including alerts export and webhooks.
