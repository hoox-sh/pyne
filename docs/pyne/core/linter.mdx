...
| --- | --- | --- |
| `E001` | error | `parse(source, filename)` raises any exception |
...
| `W002` | Version integer `< 5` (deprecated; suggest v6) |
...
| `W102` | *(retired)* `plot(… style=plot.style_histogram` | Was: consider `plotcandle`. Code kept greppable; the rule no longer fires. |
| `W103` | *(retired)* `var int name = na` | Was: prefer `0` for type safety. Code kept greppable; the rule no longer fires. |
...
| Issue | Explanation |
| --- | --- |
| `C003` on legitimate indented `if` blocks | Rule retired in 0.6.0; the code no longer fires (was a coarse `^\s+if\s+` heuristic) |
| `C001` noise | Many valid snake_case or lower identifiers |
| `E001` still not a caret dump | Linter stores the short `.message` plus line/column; catch `pynescript.ast.error.SyntaxError` for the full caret `__str__` |
