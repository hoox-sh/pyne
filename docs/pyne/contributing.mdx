...
Report artifact: `.cache/interp_compile_parity.json`. Value mismatches on shared `series` keys are regressions. First-party `hline` / `fill` / `bgcolor` / `plotshape` / `barcolor` / `plotarrow` / `plotbar` / `plotcandle` keys match (0.3.12); `drawings` is geometry-only. `--ignore-hline-keys` / `--ignore-fill-keys` are optional for leftover corpus noise. End-user mode semantics: [Evaluate scripts — Runtime modes](/pyne/docs/enduser/guides/evaluate-scripts#runtime-modes-interpret--compile--auto).
...
| TS library | sister repo [hoox-sh/pynets](https://github.com/hoox-sh/pynets) / `@hoox-sh/pynets` |
...
- Do not copy PyneTS sources into this tree; work in the standalone [hoox-sh/pynets](https://github.com/hoox-sh/pynets) repo / `@hoox-sh/pynets` package
