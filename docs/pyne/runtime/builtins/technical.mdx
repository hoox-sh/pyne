...
| Family | Incremental builtins (non-exhaustive) |
| --- | --- |
| Moving averages | `sma`, `ema`, `rma`, `wma`, `hma`, `vwma`, `swma`, `kama`, `dema`, `tema`, `alma` |
| Oscillators | `rsi`, `macd`, `stoch`/`stochrsi`, `cci`, `cmo`, `tsi`, `roc`, `wpr` |
| Volatility / structure | `atr`, `tr`, `stdev`, `bb`, `highest`/`lowest`, `linreg`, `adx`/`dmi`, `supertrend`, `kc`, `sar` |
| Volume (**0.3.10**) | `obv`, `wad`/`wvad`, `cmf`, `klinger`, `mfi`, `vwap`, `accdist` |
| Other | `change`/`mom`/`cum`, `median`, `percentrank`, `rising`/`falling`, `barssince`, pivots |

Residual full-list helpers include `ta.nvi` / `ta.pvi`. **`ta.atr` is Wilder RMA of true range** on interpret (full + incremental) and Numba (`ta.rma(ta.tr, length)` — 0.3.4+).
