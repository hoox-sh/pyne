...
| Oscillators | `rsi`, `macd`, `stoch`/`stochrsi`, `cci`, `cmo`, `tsi`, `roc`, `wpr`, `aroon`, `dpo`, `kst` |
| Volatility / structure | `atr`, `tr`, `stdev`, `bb`, `highest`/`lowest`, `linreg`, `adx`/`dmi`, `supertrend`, `kc`, `sar`, `donchian` |
