...
| Volume (**0.3.10**) | `obv`, `wad`/`wvad`, `cmf`, `klinger`, `mfi`, `vwap` |
| Other | `change`/`mom`/`cum`, `median`, `percentrank`, `rising`/`falling`, `barssince`, pivots |

Residual full-list helpers include `ta.nvi` / `ta.pvi`. ATR remains the current EMA-of-TR oracle (not a Wilder re-baseline — roadmap **F1**).
