...
4. **Foreign + pre-evaluated expression** (UDF result, list/tuple of chart values, non-string expr) without multi-symbol data → **`na`** (not chart close as “dividends”).
5. Fundamental / non-equity prefixes (`DIVIDEND`, `FACTSET`, `EARNINGS`, `ESD_`) with no feed hit → **`na`** (no mock OHLCV).
6. Bare equity-style string names may still use **legacy mock** prices for offline demos when no feed is wired.
...
| Complex third arg (UDFs, `year_sum(close)`, `ta.*`, arithmetic) | `np.nan` |
...
| `backend/runtime.py` | Feed wiring; `_compile_eligible` / `_run_auto` |
...
### Multi-timeframe close (chart symbol)
...
Same-symbol: interpret uses chart provider / series; compile may passthrough `close`. With a multi-TF provider, daily closes come from the feed.
