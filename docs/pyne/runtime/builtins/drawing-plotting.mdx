...
| `plotshape` / `plotchar` / `plotarrow` | markers | Interpret: bool/condition series + style/location/char meta. Compile: `__drawings` events (object mode) |
...
Hosts often **also** capture per-bar plot values on the evaluator (`plot_outputs` / `_plot_value_cols`) for time-aligned series maps—see `backend/runtime.py` and `backend/evaluator.py`.
...
Interpret (`backend/evaluator.py` + Runtime packaging):
...
Interpret packaging lives in `backend/runtime.py` / `backend/evaluator.py`; compile emits matching series keys + `__drawings` fill events.
...
| `backend/evaluator.py` | Host plot capture: hline / fill / bgcolor / plotshape series + meta |
| `backend/runtime.py` | JSON packaging: series keys, stub nulling, hline fill-forward, fill plot refs |
...
| `label` / `line` / `box` / … | Handle dict + `__drawings`; set_* mutators recorded; delete is MVP no-op | Full registry, GC caps, last-bar helpers |
...
5. **Compile path.** Numeric mode supports `plot` arrays; drawings force object mode with a simplified event list—not full delete/style/GC parity with the interpreter. Prefer interpret when you need bgcolor/plotshape **series** columns, not only drawing events.
