...
4. **Compile path.** Numeric mode supports `plot` arrays; drawings force object mode with a simplified event list—not full delete/style parity with the interpreter. Prefer interpret when you need bgcolor/plotshape **series** columns, not only drawing events.
5. **Import stubs → null.** `__pine_import_stub__` and `"<PineImportStub…>"` strings never appear as plot series cells after Runtime packaging.
6. **No GPU/canvas in-process.** Registries are data; AXIS/HOOX render elsewhere.
