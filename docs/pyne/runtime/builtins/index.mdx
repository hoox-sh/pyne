...
| `alert` / logging | `AlertsMixin`, `register_logging_functions` | [Alerts](/pyne/docs/runtime/alerts) |
...
Full detail: [Alerts](/pyne/docs/runtime/alerts).
...
3. **Bar mode.** Stateful `ta.crossover` / similar use per-bar call indices reset by the host (`_cross_call_i`). Incremental `ta.*` (`PYNE_TA_INCREMENTAL`, default on) uses a sibling `_ta_call_i` slot.
4. **Mock fallbacks.** Bare equity-style `request.security` strings may still mock offline; foreign / fundamental prefixes and complex expressions return **`na`** (see [request / input](/pyne/docs/runtime/builtins/request-input)).
