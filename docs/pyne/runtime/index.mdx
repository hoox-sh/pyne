...
**`request.security` foreign-na policy (landed):** same-symbol simple OHLCV may passthrough; foreign symbols and complex security expressions resolve to **`na`** on both interpret and compile (no inventing chart close as foreign fundamentals). Real multi-symbol feeds remain adapter work (roadmap **B1**).
...
# Runtime(symbol="CHART").run(source, ohlcv_data=bars, mode="auto", timeout_seconds=None)
```

Hosts that own OHLCV (Pro API, workers) use the package host: push bar → fill pending orders → `visit(tree)` → drain events → collect plots. SoT: `src/pynescript/runtime/`; `backend.runtime` is a compat shim.
...
| `backend/runtime.py` | Compat re-export of package Runtime for Pro API |
...
5. **Incremental TA.** Hot-path kernels (`sma`/`ema`/`rsi`/`macd`/`atr`/… plus R7 `bb`/`kama`/`cmo`/`stochrsi`) use call-site incremental updates in bar mode (`PYNE_TA_INCREMENTAL=0` to disable). Residual nested full-list paths may still recompute (**T2** partial).
