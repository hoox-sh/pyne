...
Pro API / `backend.runtime.Runtime` optionally **trims** chronological `current_series` lists and related host history so long charts do not grow \(O(\text{bars})\) memory per series.
...
**Correctness notes** (see `backend/series.py`):
...
| `backend/runtime.py` | `PineSeries.update` per bar; series-cap trim |
| `backend/series.py` | `PineSeries`, `resolve_series_cap`, `PYNE_SERIES_CAP` / `PYNE_SERIES_MAX` |
...
2. **No negative Pine indices.** Soft-fail to `na` / `nan` (not Python wraparound)—different from Python lists. See highestbars note above.
...
6. **Tuple returns from multi-value `ta.*`.** Unpacking `[a, b, c] = ta.macd(...)` uses `current` when it is a sequence of matching arity; otherwise history heuristics apply.
7. **Host series assign is by value.** New names never share the host OHLCV/`time` buffer reference.
