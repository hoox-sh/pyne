...
flowchart LR
  ...
  Dict --> TS[PyneTS RuntimeResult.events]
  Dict --> HOOX[pyne-worker → trade-worker]
```
...
| `pynets/src/runtime/interpret.ts` `StrategyEvent` | TS library twin |
...
5. **Not alerts.** `alert()` / `alertcondition()` use a separate [alerts](/pyne/docs/runtime/alerts) channel—not `StrategyEvent`.
