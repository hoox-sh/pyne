...
This is an MVP with growing surface—not a full substitute for the interpreter on every script (notably multi-asset `request.*` and full strategy analytics). **0.3.16** recovered a large object-mode residual class: UDF locals, nopython `None`/unicode helpers, matrix handles, UDT returns, statement `switch` assign, and drawing/`chart.point` copy. Plot-value MISMATCH vs interpret (**P1p**) remains a residual, not a missing language surface.
