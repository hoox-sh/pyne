...
This is an MVP with growing surface—not a full substitute for the interpreter on every script (notably multi-asset `request.*` and full strategy analytics).
...
flowchart LR
  ...
  CV -->|no UDT/map/draw/strategy| Num["@numba.njit loop"]
...
Host packing lives in `engine.CompiledScript.run` (and `backend.runtime.Runtime._run_compiled`).
...
- Map operations
- Drawing APIs / certain plot variants (`hline`, `fill`, …)
...
| `hline(...)` | Object mode. Exports a **constant numeric series** under a uniquified title (`hline`, `hline_2`, …) **and** a `__drawings` event (interpret packaging parity) |
| `fill(..., title=…)` | Object mode. Exports a series key (often **all-null** / float64 `nan`) **plus** `__drawings` so AXIS / key-set parity with interpret holds; band color lives on the drawing/meta side |
...
| `backend/runtime.py` | `_run_compiled` envelope → plots/series; passes `time=` |
...
2. **Disk IR** (default on): modules under `PYNE_COMPILE_CACHE_DIR` or `$XDG_CACHE_HOME/pynescript/compile`.
...
Writes `.cache/interp_compile_parity.json`. Use this harness when changing emitters or kernels; prefer it over ad-hoc one-offs for structural key drift (`hline` / `fill` series).
...
Expand Numba `ta.*` surface, richer drawings, nested UDT methods, broader `request.*` (still interpret-first for multi-asset), and grow systematic interpret↔compile parity coverage per lowered construct. In-process + disk IR cache and product prewarm are landed. See `docs/COMPILER_PLAN.md` for the living checklist.
