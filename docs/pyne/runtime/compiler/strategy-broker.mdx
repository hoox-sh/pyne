...
sequenceDiagram
  ...
  Loop->>B: begin_bar(idx, OHLC, time)
  Note over B: set_bar + process_pending when book nonempty
...
1. **Same order as interpreter:** `begin_bar` (or `set_bar` → `process_pending_orders`) → script body.
2. **Closed-trade list is partial.** The compile broker now keeps `closed_trade_records` and some `closedtrades_*` accessors (profit, comments, max drawdown/runup). Prefer interpret for the full `strategy.closedtrades.*` / `opentrades.*` metric surface.
