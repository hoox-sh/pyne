...
**Numeric mode** targets the common indicator subset—series assignments, arithmetic and logical operators, history access (`close[n]`), control flow (`if`, `for`, `while`), selected `ta.*` routines (`sma`, `ema`, `rsi`, `highest`, `lowest`), scalar math helpers, `plot`, `input.*` defaults, and `var`/`varip` persistence—executed under `@numba.njit` when Numba is installed.
