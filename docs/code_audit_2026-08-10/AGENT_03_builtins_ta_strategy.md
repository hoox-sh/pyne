...
2. **`ta.atr` is EMA-of-TR** (shared with compile `numba_atr`), not reference Pine's documented **`rma(tr)` / Wilder** definition — dual-host OK, reference Pine parity risk High.
...
**reference semantics:** `strategy.exit` places protective orders that fill on subsequent bars when price path hits stop/limit (often with OCA between legs). Instant oracle fill is only valid for unit/parity fixtures that intentionally model “exit called ⇒ closed”.

**Impact:** Backtest equity, hold times, and multi-bar TP/SL strategies diverge from reference Pine. Documented as intentional “interpret oracle” in tests ([`tests/test_compiler_strategy.py:309–321`](../../tests/test_compiler_strategy.py)).

**Related gaps in same handler:**

- `from_entry` is documented but **not used** to select which open-trade leg to close.
- `profit` / `loss` kwargs are coerced as **prices** (`limit`/`stop` aliases), not Pine tick offsets from entry.
...
### C2 — `ta.atr` formula is EMA-of-TR, not Wilder RMA (reference-semantics mismatch)
...
Reference Pine defines ATR as **`ta.rma(ta.tr(true), length)`** (Wilder smoothing, α = 1/length). ADX/DMI paths in the same codebase correctly use RMA (`_rma_state_step`, `numba` RMA helpers), so the stack **knows** Wilder — ATR simply doesn’t use it.

**Impact:** Supertrend, Keltner, ATR stops, and any script using `ta.atr` will systematically differ from reference charts while still matching interpret↔compile. Residual mismatches noted for Supertrend in parity work ([`docs/parity_round8/AGENT_05_interpret_ta.md`](../parity_round8/AGENT_05_interpret_ta.md)).
...
Incremental EMA docstring claims “matches numba_ema_inc / reference” (SMA seed). Full-history and MACD still use first-value seed. Unit tests that force full recompute (`_pine_ta_incremental=False` or non-bar mode) diverge from bar-mode Runtime.
...
**Recommendation:** Unify seed to SMA-window (reference) for `_ema`, `_ema_state_step`, and MACD signal EMA; keep single formula source with numba.
...
**Impact:** Scripts that re-call `strategy.entry("L", …)` every bar behave like cancel/replace (reference-like for same id) but differ from order-fill averaging — easy to misread. Documented in `_open_position_qty` docstring; still a parity footgun for hybrid entry APIs.
...
[`oscillators.py:50–87`](../../src/pynescript/ast/evaluator/builtins/technical_submodules/oscillators.py): primary path returns **%K only**; 5-arg legacy smooth uses EMA for %D. Reference Pine `ta.stoch` returns single %K (OK); community dual-output forms are partial.
...
**Risks:** Array stats skip non-numeric (reference-like); `array.size` soft-na on non-array ([`arrays.py:296–308`](../../src/pynescript/ast/evaluator/builtins/arrays.py)) can hide type bugs; matrix `avg_row` returns 0 on empty numeric filter rather than na.
...
### L1 — Community / pseudo-reference `ta.*` surface sprawl
...
[`docs/numerical_validation_report.md`](../numerical_validation_report.md) claims ~reference identity; dual-host alignment notes are more honest. ATR EMA choice means the report should not be read as ATR reference gold.
...
| Per-function reference form variants (arity overloads) | **Good** in oscillators/MAs (stoch, cci, vwma) |
...
2. Explicit “ATR is EMA-of-TR (reference Pine uses RMA)” with dual-host rationale.
...
- **Drawing GC** with declaration caps (reference-like).
...
| --- | --- | --- |
| TA formula correctness vs reference Pine | **6.5** | RSI/RMA/SMA strict-window strong; ATR EMA, EMA seed splits, residual HMA/VWAP |
...
| **Blended** | **~7.4** | Production-capable host, not full reference-platform backtest clone |
...
| --- | --- | --- | --- |
| **P0** | Pending `strategy.exit` (stop/limit/OCA); stop immediate close when not triggered | L | Critical reference/backtest correctness |
| **P0** | ATR → Wilder RMA in interpret + numba + Supertrend/KC dependents | M | Formula bug vs reference Pine; dual-host can stay locked |
