...
``PineSeries`` is newest-first (``history[0]`` == current). Host
:class:`~pynescript.runtime.Runtime` keeps separate chronological
``current_series`` lists for ``ta.*`` helpers; those lists are optionally
capped to ``max_bars_back`` / ``_SERIES_MAX`` so long charts do not grow
O(bars) memory per series.
