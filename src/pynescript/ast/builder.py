...
Converts ANTLR parse trees generated by the Pinescript grammar into
the internal AST representation using ASDL-generated node classes.

Key Classes:
- PinescriptASTLocator: Extracts position information (lineno, col_offset) from parse tree tokens
- PinescriptCommentParser: Parses Pine Script comment annotations (//@version, //@description, etc.)
- PinescriptASTBuilder: Main visitor that builds AST nodes from parse tree contexts
  (inherits from PinescriptParserVisitor and implements visit_* methods for each grammar rule)

The builder handles:
- Expression parsing (literals, operators, function calls, subscripts)
- Statement parsing (assignments, if/else, loops, function definitions)
- Type definitions and user-defined types (UDT)
- Special Pine Script constructs (strategies, studies, indicators)
"""
...
class PinescriptCommentParser:
    ...
    def _parseComment(self, comment: str) -> tuple[str, tuple[str, ...]]:  # noqa: C901
        ...
        """
        # Try to match assignment-style annotation: //@version = 5
        ...
        if m:
            ...
            if parts[0] in {"version"}:
                kind += "S"
            return kind, parts
        # Try to match simple annotation: //@description "text"
