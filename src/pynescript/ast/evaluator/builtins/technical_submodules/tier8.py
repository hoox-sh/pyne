...
class Tier8Indicators(TechnicalHelpers):
    ...
    def _builtin_ta_intelligent_strategy_synthesizer(self, args: list[Any]) -> dict[str, Any]:
        ...
        raw_signal = (t * 0.4) + (m * 0.3) + (vol * 0.2) + (v * 0.1 * (1 if t > 0 else -1)) # Volatility direction depends on trend
        
        # Adjust signal based on regime alignment
        regime_alignment = 50.0
        if market_regime == "trending_up":
            if t > 0: regime_alignment += 30
            if m > 0: regime_alignment += 20
        elif market_regime == "trending_down":
            if t < 0: regime_alignment += 30
            if m < 0: regime_alignment += 20
        elif market_regime == "ranging":
            if abs(t) < 0.3: regime_alignment += 30
            if abs(m) < 0.3: regime_alignment += 20
        elif market_regime == "volatile":
            if v > 0.7: regime_alignment += 40
        elif market_regime == "dead":
            if v < 0.2 and vol < 0.2: regime_alignment += 40
